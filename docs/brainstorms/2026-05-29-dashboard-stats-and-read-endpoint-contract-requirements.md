...
> **Superseded after doc review (2026-05-30).** R10–R12 were reversed when doc review confirmed that `packages/frontend/src/lib/event-routing.ts` already invalidates `['dashboard-stats']` on `agent_status`, `campaign_status`, `task_update`, and `crack_result`. Adding a new event type would have produced redundant double-invalidations at every publisher site with no behavioral payoff. See [`docs/plans/2026-05-29-002-feat-dashboard-stats-and-contract-plan.md`](../plans/2026-05-29-002-feat-dashboard-stats-and-contract-plan.md) §Reframed After Doc Review for the full rationale. The freshness behavior R10–R12 specified is delivered today by the existing routing map; the only behavioral change that shipped was the 30s → 60s polling-floor reduction (D5). The text below is preserved as the historical record of what the brainstorm proposed.
