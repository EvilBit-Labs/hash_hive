...
export function createHeartbeatMonitorWorker(connection: Redis): Worker<HeartbeatMonitorJob> {
  const worker = new Worker<HeartbeatMonitorJob>(
    ...
    async (job) => {
      logger.info({ jobId: job.id, triggeredAt: job.data.triggeredAt }, 'Running heartbeat monitor')
      ...
      const anyMovement =
        ...
        result.errored > 0
      if (anyMovement) {
        const isWarn = result.failedOverrun > 0 || result.failedMaxRetries > 0 || result.errored > 0
        const summary = {
