...
import { claimEnrollmentToken } from '../../services/enrollment-tokens.js'
...
//
// Anonymous BY OMISSION: there is deliberately no `agentRoutes.use('/enroll',
// ...)` line above, and the route declares `security: []`. A brand-new
// agent has no bearer token yet — it presents the operator-minted
// enrollment token (`etk_<id>_<phrase>`) plus a stable, self-generated
// `clientId`, and receives its long-lived bearer token (`agt_<id>_<random>`)
  ...
  try {
    ...
    if (!result.ok) {
      ...
      if (result.reason === 'invalid') {
        return c.json(
          {
            error: { code: 'ENROLLMENT_REJECTED', message: 'Invalid or revoked enrollment token' },
          },
          401
        )
      }
