...
export type LineCountResourceType = 'wordlist' | 'rulelist'
...
/**
 * For an attack's referenced wordlist/rulelist, enqueue a count job for any that
 * still lack a line count so the attack's keyspace can eventually be computed.
 * No-op for resources already counted (the common case) and for mask-only
 * attacks. Best-effort end to end.
 */
  ...
  try {
    ...
    for (const [resourceType, resourceId] of checks) {
      ...
      const [row] = await db
        ...
        .limit(1)
      if (row && (row.lineCount === null || row.lineCount === undefined)) {
